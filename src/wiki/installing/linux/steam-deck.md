...
- If you want Minecraft to launch without the PolyMC launcher add `-l <instance-id>` in your PolyMC's launch options in steam, if you use this launch option make sure to disable `Close PolyMC after game window opens` in PolyMC so that the PolyMC does not open when the game is closed, find out more about CLI launch options [here](https://polymc.org/wiki/getting-started/command-line-interface/)
